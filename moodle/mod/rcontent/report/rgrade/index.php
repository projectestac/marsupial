...
if (!$book) {
	print_error('Book not valid');
}

require_login($courseid, false);
$PAGE->set_url(new moodle_url('/mod/rcontent/report/rgrade/index.php', array('id' => $id)));
...
$page_title = $mod_title . ': ' . $rcontent->name . ' ('.$book->name.')';
$title = $mod_title . ': ' . $rcontent->name . ' ('.$book->name.')';

$PAGE->set_heading($page_title);
$PAGE->set_title($title);

$jspath = $CFG->wwwroot.'/mod/rcontent/report/rgrade/js';
...
$csspath = $CFG->wwwroot.'/mod/rcontent/report/rgrade/css';
...
$PAGE->navbar->add(get_string('pluginname', 'rcontent'), new moodle_url('/mod/rcontent/index.php', array('id' => $courseid)));
$PAGE->navbar->add($rcontent->name . ' ('.$book->name.')', new moodle_url('/mod/rcontent/view.php', array('id' => $id)));
$PAGE->navbar->add($mod_title, null);

$PAGE->add_body_class("blocks-rgrade");

echo $OUTPUT->header();
?>
<form id="rgrade_search_form" action="#"></form>

<div id="rgrade_table"><div id="combo" class="clearfix"></div></div>
