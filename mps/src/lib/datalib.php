...
function get_courses_search($searchterms, $sort='fullname ASC', $page=0, $recordsperpage=50, &$totalcount) {
    ...
    if ($rs = get_recordset_sql($sql)) {
        ...
        while ($course = rs_fetch_next_record($rs)) {
            $course = make_context_subobj($course);
// IECISA ************ MODIFICATED -> to prevent show hidden courses to non editing teacher
            //if ($course->visible || has_capability('moodle/course:viewhiddencourses', $course->context)) {
            if ($course->visible || (has_capability('moodle/course:viewhiddencourses', $course->context) && has_capability('moodle/legacy:editingteacher', $course->context, $USER->id, false)) || has_capability('moodle/legacy:admin', $course->context, $USER->id, false)) {
//*********** END
...
// IECISA -> MPS *********** MODIFIED -> we will use our own function
/**
